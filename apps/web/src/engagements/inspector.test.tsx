...
describe("surface inspector", () => {
  ...
  it("renders stub actions disabled with honest reasons by default", async () => {
    stubFindings([]);
    renderInspector(probeInspectorRecord(probe, engagementId));

    const lead = await screen.findByRole("button", { name: "Start a lead" });
    ...
    expect(ask.getAttribute("title")).toBe("Advisor follow-ups arrive in a later slice.");
