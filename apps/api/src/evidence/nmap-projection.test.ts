...
async function writeFile(dir: string, id: string, xml: Buffer) {
  ...
  await fh.write(xml); await fh.sync(); await fh.close(); chmodSync(p, 0o600);
  return digest;
}
...
function addArtifact(
  db: ReturnType<typeof openEngagementDatabase>,
  aId: string,
  rId: string,
  xml: Buffer,
  digest: string,
...
describe("nmap projection", () => {
  it("projects valid/empty, skips partial/truncated, rejects malformed and is idempotent", async () => {
    const f = await fixture(); const proj = new NmapProjectionService(f.store, f.repo);
  ...
  it("fails for oversize before download and for corrupted store", async () => {
    ...
    const big = NMAP_MAX_XML_BYTES + 1, dig = `sha256:${"a".repeat(64)}`;
    ...
    let called = false;
    const fake1 = { verifiedDownload: async () => { called = true; return { status: "ready" as const, sizeBytes: big, digest: dig, stream: (async function* () {})() }; } } as unknown as EvidenceStore;
  ...
  it("is engagement scoped", async () => {
    const f = await fixture(); const { buildApp } = await import("../app.js");
